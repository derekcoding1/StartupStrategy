...
from trash_V2 import isTrash  #Change Type of Trash
import datetime
import time
import os
import pandas as pd
...
# load data
datadir = 'data'
data = pd.read_stata(datadir+"/all_public_firms.dta")
...
def single_mining (X):
    ...
    #record_startup(company_id, year, text)
...
df_public = data[(data.ipoyear <= 2010)]
# with Missing value df_public = data[(data.ipoyear <= 2010) | (pd.isna(data.ipoyear))]
#df_public = df_public.iloc[0:100,:]    


path = os.getcwd() + '/data/public_data'
